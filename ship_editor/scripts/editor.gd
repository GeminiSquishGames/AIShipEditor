...
func _create_parts_list():
    for part_type in part_data.keys():
        ...
        part_button.custom_minimum_size = Vector2i(0, 50)
        ...
        part_icon.custom_minimum_size = Vector2i(40, 40)
...
func _load_ship(ship_name: String):
    ...
    # Center the ship
    ship_sprite.position = Vector2i.ZERO
    camera.position = Vector2i.ZERO
    ...
    for y in range(0, image_size.y, grid_cell_size):
        for x in range(0, image_size.x, grid_cell_size):
            ...
            if cell_has_pixels:
                ...
                var cell_position = Vector2i(x, y) + Vector2i(grid_cell_size / 2, grid_cell_size / 2) - (image_size / 2) as Vector2i
                ...
                grid_rect.size = Vector2i(grid_cell_size, grid_cell_size)
                grid_rect.position = Vector2i(x, y) - image_size / 2
...
func _get_mouse_position_in_viewport() -> Vector2i:
...
func _try_select_part_at_position(position: Vector2i):
    var closest_part_index = -1
    var closest_distance = grid_cell_size

    for i in range(placed_parts.size()):
        var part = placed_parts[i]
        var part_pos = Vector2i(part.position)  # Convert Vector2i to Vector2i if needed
...
func _try_place_part(position: Vector2i):
...
func _finish_move_part(position: Vector2i):
...
func _find_closest_grid_cell(position: Vector2i) -> Vector2i:
    var closest_cell = null
    var closest_distance = INF

    for cell in grid_cells:
        var distance = position.distance_to(Vector2i(cell))
        if distance < closest_distance and distance < grid_cell_size * 1.5:
            closest_cell = cell
            closest_distance = distance

    if closest_cell == null:
        closest_cell = Vector2i.ZERO
